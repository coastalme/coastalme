...
bool CSimulation::bReadIniFile(void)
{
   ...
   {
      // if user has supplied home directory replace cme run directory with user
      // supplied dir
   ...
   while (getline(InStream, strRec))
   {
      ...
      if ((! strRec.empty()) && (strRec[0] != QUOTE1) && (strRec[0] != QUOTE2))
      {
         ...
         // Find the colon: note that lines MUST have a colon separating data from
         // leading description portion
         size_t nPos = strRec.find(COLON);

         if (nPos == string::npos)
         {
            // Error: badly formatted (no colon)
            cerr << ERR << "on line " << nLine << ": badly formatted (no ':') in "
                 << strFilePathName << endl
                 << "'" << strRec << "'" << endl;
            return false;
         }
         ...
         // Look for a trailing comment, if found then terminate string at that
         // point and trim off any trailing whitespace
         ...
         switch (i)
         {
            case 1:
               ...
               if (strRH.empty())
                  strErr =
                     "line " + to_string(nLine) + ": path and name of main datafile";

               else
               {
                  // First check that we don't already have an input run-data filename,
                  // e.g. one entered on the command-line
                  if (m_strDataPathName.empty())
                  {
                     // We don't: so first check for leading slash, or leading Unix home
                     // dir symbol, or occurrence of a drive letter
                     if ((strRH[0] == PATH_SEPARATOR) || (strRH[0] == TILDE) ||
                         (strRH[1] == COLON))
            ...
            case 2:
               ...
               {
                  // Check for trailing slash on CoastalME output directory name (is
                  // vital)
                  if (strRH[strRH.size() - 1] != PATH_SEPARATOR)
                     strRH.push_back(PATH_SEPARATOR);

                  // Now check for leading slash, or leading Unix home dir symbol, or
                  // occurrence of a drive letter
                  if ((strRH[0] == PATH_SEPARATOR) || (strRH[0] == TILDE) ||
                      (strRH[1] == COLON))
            ...
            case 3:
               ...
               if (! strRH.empty())
               {
                  ...
                  if (strRH.find('@') == string::npos)
                     strErr =
                        "line " + to_string(nLine) + ": email address for messages";
...
bool CSimulation::bReadRunDataFile(void)
{
   ...
   if (! InStream.is_open())
   {
      ...
      cerr << ERR << "cannot open " << m_strDataPathName << " for input"
           << endl;
      return false;
   }
   ...
   while (getline(InStream, strRec))
   {
      ...
      if ((! strRec.empty()) && (strRec[0] != QUOTE1) && (strRec[0] != QUOTE2))
      {
         ...
         // Find the colon: note that lines MUST have a colon separating data
         // from leading description portion
         nPos = strRec.find(COLON);

         if (nPos == string::npos)
         {
            // Error: badly formatted (no colon)
            cerr << ERR << "on line " << to_string(nLine)
                 << "badly formatted (no ':') in " << m_strDataPathName << endl
                 << strRec << endl;
            return false;
         }
         ...
         // Look for trailing comments, if found then terminate string at that
         // point and trim off any trailing whitespace
         bool bFound = true;
         ...
         switch (i)
         {
            ...
            case 4:
               // Get the start date/time of the simulation, format is [hh-mm-ss
               // dd/mm/yyyy]
            ...
            case 5:
               // Duration of simulation (in hours, days, months, or years): sort out
               // multiplier and user units, as used in the per-timestep output
               strRH = strToLower(&strRH);
               ...
               // And now calculate the duration of the simulation in hours: first
               // find whitespace between the number and the unit
            ...
            case 6:
               ...
               {
                  strErr =
                     "line " + to_string(nLine) + ": units for simulation timestep";
                  break;
               }

               // we have the multiplier, now calculate the timestep in hours: look
               // for the whitespace between the number and unit
               nPos = strRH.rfind(SPACE);

               if (nPos == string::npos)
               {
                  strErr =
                     "line " + to_string(nLine) + ": format of simulation timestep";
                  break;
               }
            ...
            case 7:
            {
               ...
               // if groups are comma-separated e.g., "6 12 24 48 hours, 1 2 6
               // months, 1 years"
               strRH = strToLower(&strRH);
               ...
               do
               {
                  ...
                  {
                     strErr = "line " + to_string(nLine) +
                        ": format of save times/intervals in group '" +
                        strGroup + "'";
                     break;
                  }
                  ...
                  do
                  {
                     ...
                     string const strNumber =
                        strNumbers.substr(nSpacePos, nNextSpace - nSpacePos);
                     ...
                     if (! strNumber.empty())
                     {
                        ...
                        {
                           strErr =
                              "line " + to_string(nLine) + ": too many save intervals";
                           break;
                        }
            ...
            case 8:
               ...
               if (strRH.empty())
               {
                  // User didn't specify a random number seed, so seed with a real
                  // random value, if available
                  random_device rdev;
                  m_ulRandSeed[0] = rdev();
               ...
               {
                  // User did specify at least one random number seed. Next find out
                  // whether we're dealing with a single seed or more than one: check
                  // for a space
                  ...
                  else
                  {
                     ...
                     // If we haven't filled all random number seeds, make all the
                     // remainder the same as the last one read
            ...
            case 9:
               ...
               {
                  strErr = "line " + to_string(nLine) +
                     ": invalid integer for max save digits for GIS output "
                     "file names '" +
            ...
            case 10:
               // Save digits for GIS output sequential or iteration number?    [s =
               // sequential, i = iteration]: s
               if (strRH.empty())
                  strErr = "line " + to_string(nLine) +
                     ": must specify save digits for GIS output as sequential "
                     "or as iteration number";
               ...
               else
               {
                  ...
                  else
                  {
                     ...
                     strErr +=
                        ": invalid code for save digits for GIS output save "
                        "number (must be s or i)";
            ...
            case 11:
               ...
               else
               {
                  ...
                  {
                     // Set switches for all GIS raster output. Some of these (e.g. all
                     // relating to fine sediment) are ignored if e.g. no fine sediment
                     // layers are read in
                     m_bSuspSedSave = true;
                  ...
                  {
                     // Set switches for usual GIS raster output. Again, some of these
                     // (e.g. all relating to fine sediment) are ignored if they are
                     // irrelevant
                     m_bSuspSedSave = true;
                     m_bAvgSuspSedSave = true;
                     m_bFineUnconsSedSave = true;
                  ...
                  {
                     // We are not outputting either the "usual" or the "all"
                     // collections of raster GIS files, so set switches (and remove
                     // strings) for those optional files for which the user specified
                     // the code
                     ...
                     if (strRH.find(RASTER_POTENTIAL_PLATFORM_EROSION_CODE) !=
                         string::npos)
                     {
                        m_bPotentialPlatformErosionSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &RASTER_POTENTIAL_PLATFORM_EROSION_CODE);
                     }

                     if (strRH.find(RASTER_ACTUAL_PLATFORM_EROSION_CODE) !=
                         string::npos)
                     {
                        m_bActualPlatformErosionSave = true;
                        strRH = strRemoveSubstr(&strRH,
                                                &RASTER_ACTUAL_PLATFORM_EROSION_CODE);
                     }

                     if (strRH.find(RASTER_TOTAL_POTENTIAL_PLATFORM_EROSION_CODE) !=
                         string::npos)
                     {
                        m_bTotalPotentialPlatformErosionSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &RASTER_TOTAL_POTENTIAL_PLATFORM_EROSION_CODE);
                     }

                     if (strRH.find(RASTER_TOTAL_ACTUAL_PLATFORM_EROSION_CODE) !=
                         string::npos)
                     {
                        m_bTotalActualPlatformErosionSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &RASTER_TOTAL_ACTUAL_PLATFORM_EROSION_CODE);
                     }

                     if (strRH.find(RASTER_POTENTIAL_BEACH_EROSION_CODE) !=
                         string::npos)
                     {
                        m_bPotentialBeachErosionSave = true;
                        strRH = strRemoveSubstr(&strRH,
                     ...
                     if (strRH.find(RASTER_ACTUAL_BEACH_EROSION_CODE) !=
                         string::npos)
                     {
                        m_bActualBeachErosionSave = true;
                        strRH =
                           strRemoveSubstr(&strRH, &RASTER_ACTUAL_BEACH_EROSION_CODE);
                     }

                     if (strRH.find(RASTER_TOTAL_POTENTIAL_BEACH_EROSION_CODE) !=
                         string::npos)
                     {
                        m_bTotalPotentialBeachErosionSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &RASTER_TOTAL_POTENTIAL_BEACH_EROSION_CODE);
                     }

                     if (strRH.find(RASTER_TOTAL_ACTUAL_BEACH_EROSION_CODE) !=
                         string::npos)
                     {
                        m_bTotalActualBeachErosionSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &RASTER_TOTAL_ACTUAL_BEACH_EROSION_CODE);
                     }
                     ...
                     if (strRH.find(RASTER_AVG_WAVE_ORIENTATION_CODE) !=
                         string::npos)
                     {
                        m_bAvgWaveAngleSave = true;
                        strRH =
                           strRemoveSubstr(&strRH, &RASTER_AVG_WAVE_ORIENTATION_CODE);
                     }
                     ...
                     if (strRH.find(RASTER_BASEMENT_ELEVATION_CODE) != string::npos)
                     {
                        m_bBasementElevSave = true;
                        strRH =
                           strRemoveSubstr(&strRH, &RASTER_BASEMENT_ELEVATION_CODE);
                     }
                     ...
                     if (strRH.find(RASTER_CLIFF_COLLAPSE_EROSION_FINE_CODE) !=
                         string::npos)
                     {
                        m_bCliffCollapseSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &RASTER_CLIFF_COLLAPSE_EROSION_FINE_CODE);
                     }

                     if (strRH.find(RASTER_CLIFF_COLLAPSE_EROSION_SAND_CODE) !=
                         string::npos)
                     {
                        m_bCliffCollapseSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &RASTER_CLIFF_COLLAPSE_EROSION_SAND_CODE);
                     }

                     if (strRH.find(RASTER_CLIFF_COLLAPSE_EROSION_COARSE_CODE) !=
                         string::npos)
                     {
                        m_bCliffCollapseSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &RASTER_CLIFF_COLLAPSE_EROSION_COARSE_CODE);
                     }

                     if (strRH.find(RASTER_TOTAL_CLIFF_COLLAPSE_EROSION_FINE_CODE) !=
                         string::npos)
                     {
                        m_bTotCliffCollapseSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &RASTER_TOTAL_CLIFF_COLLAPSE_EROSION_FINE_CODE);
                     }

                     if (strRH.find(RASTER_TOTAL_CLIFF_COLLAPSE_EROSION_SAND_CODE) !=
                         string::npos)
                     {
                        m_bTotCliffCollapseSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &RASTER_TOTAL_CLIFF_COLLAPSE_EROSION_SAND_CODE);
                     }

                     if (strRH.find(RASTER_TOTAL_CLIFF_COLLAPSE_EROSION_COARSE_CODE) !=
                         string::npos)
                     {
                        m_bTotCliffCollapseSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &RASTER_TOTAL_CLIFF_COLLAPSE_EROSION_COARSE_CODE);
                     }

                     if (strRH.find(RASTER_CLIFF_COLLAPSE_DEPOSITION_SAND_CODE) !=
                         string::npos)
                     {
                        m_bCliffCollapseDepositionSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &RASTER_CLIFF_COLLAPSE_DEPOSITION_SAND_CODE);
                     }

                     if (strRH.find(RASTER_CLIFF_COLLAPSE_DEPOSITION_COARSE_CODE) !=
                         string::npos)
                     {
                        m_bCliffCollapseDepositionSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &RASTER_CLIFF_COLLAPSE_DEPOSITION_COARSE_CODE);
                     }
                         ...
                         string::npos)
                     {
                        m_bTotCliffCollapseDepositionSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &RASTER_TOTAL_CLIFF_COLLAPSE_DEPOSITION_SAND_CODE);
                     }
                     ...
                     if (strRH.find(RASTER_POTENTIAL_PLATFORM_EROSION_MASK_CODE) !=
                         string::npos)
                     {
                        m_bPotentialPlatformErosionMaskSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &RASTER_POTENTIAL_PLATFORM_EROSION_MASK_CODE);
                     }
                     ...
                     if (strRH.find(RASTER_INTERVENTION_CLASS_CODE) != string::npos)
                     {
                        ...
                        strRH =
                           strRemoveSubstr(&strRH, &RASTER_INTERVENTION_CLASS_CODE);
                     }
                     ...
                     if (strRH.find(RASTER_INTERVENTION_HEIGHT_CODE) != string::npos)
                     {
                        ...
                        strRH =
                           strRemoveSubstr(&strRH, &RASTER_INTERVENTION_HEIGHT_CODE);
                     }
                     ...
                     if (strRH.find(RASTER_DEEP_WATER_WAVE_ORIENTATION_CODE) !=
                         string::npos)
                     {
                        m_bDeepWaterWaveAngleSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &RASTER_DEEP_WATER_WAVE_ORIENTATION_CODE);
                     }

                     if (strRH.find(RASTER_DEEP_WATER_WAVE_HEIGHT_CODE) !=
                         string::npos)
                     {
                        m_bDeepWaterWaveHeightSave = true;
                        strRH = strRemoveSubstr(&strRH,
                     ...
                     if (strRH.find(RASTER_DEEP_WATER_WAVE_PERIOD_CODE) !=
                         string::npos)
                     {
                        m_bDeepWaterWavePeriodSave = true;
                        strRH = strRemoveSubstr(&strRH,
                     ...
                     if (strRH.find(RASTER_POLYGON_UPDRIFT_OR_DOWNDRIFT_CODE) !=
                         string::npos)
                     {
                        m_bPolygonUnconsSedUpOrDownDriftSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &RASTER_POLYGON_UPDRIFT_OR_DOWNDRIFT_CODE);
                     }

                     if (strRH.find(RASTER_POLYGON_GAIN_OR_LOSS_CODE) !=
                         string::npos)
                     {
                        m_bPolygonUnconsSedGainOrLossSave = true;
                        strRH =
                           strRemoveSubstr(&strRH, &RASTER_POLYGON_GAIN_OR_LOSS_CODE);
                     }
                     ...
                     if (strRH.find(RASTER_TOTAL_BEACH_DEPOSITION_CODE) !=
                         string::npos)
                     {
                        m_bTotalBeachDepositionSave = true;
                        strRH = strRemoveSubstr(&strRH,
                     ...
                     if (strRH.find(RASTER_SEDIMENT_INPUT_EVENT_CODE) !=
                         string::npos)
                     {
                        m_bSedimentInputEventSave = true;
                        strRH =
                           strRemoveSubstr(&strRH, &RASTER_SEDIMENT_INPUT_EVENT_CODE);
                     }

                     if (strRH.find(RASTER_SETUP_SURGE_FLOOD_MASK_CODE) !=
                         string::npos)
                     {
                     ...
                     if (strRH.find(RASTER_SETUP_SURGE_RUNUP_FLOOD_MASK_CODE) !=
                         string::npos)
                     {
                        m_bSetupSurgeRunupFloodMaskSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &RASTER_SETUP_SURGE_RUNUP_FLOOD_MASK_CODE);
                     }
                     ...
                     if (! strRH.empty())
                        strErr = "line " + to_string(nLine) + ": unknown code '" +
                           strRH + "' in list of codes for raster GIS output";
                  }
               }
            ...
            case 12:
               // Raster GIS output format (note must retain original case). Blank
               // means use same format as input DEM file (if possible)
               m_strRasterGISOutFormat = strTrimLeft(&strRH);

               // TODO 065 Remove this when GDAL gpkg raster output is working
               // correctly
               if (strRH.find("gpkg") != string::npos)
                  strErr =
                     "GDAL gpkg raster create() is not yet working correctly. "
            ...
            case 15:
               ...
               if (! strRH.empty())
               {
                  ...
                  // OK, so find out whether we're dealing with a single seed or more
                  // than one: check for a space
                  nPos = strRH.find(SPACE);
            ...
            case 16:
               ...
               else
               {
                  ...
                  if (strRH.find(VECTOR_ALL_OUTPUT_CODE) != string::npos)
                  {
                     ...
                     m_bCoastSave = m_bCliffEdgeSave = m_bWaveAngleAndHeightSave =
                        m_bNormalsSave = m_bInvalidNormalsSave = m_bAvgWaveAngleAndHeightSave =
                           m_bWaveEnergySinceCollapseSave = m_bMeanWaveEnergySave =
                              m_bBreakingWaveHeightSave = m_bCoastCurvatureSave =
                                 m_bPolygonNodeSave = m_bPolygonBoundarySave =
                                    m_bCliffNotchSave = m_bShadowBoundarySave =
                                       m_bShadowDowndriftBoundarySave =
                                          m_bDeepWaterWaveAngleAndHeightSave =
                                             m_bWaveSetupSave =
                                                m_bStormSurgeSave = m_bRunUpSave =
                  ...
                  else if (strRH.find(VECTOR_USUAL_OUTPUT_CODE) != string::npos)
                  {
                     ...
                     m_bCoastSave = m_bCliffEdgeSave = m_bWaveAngleAndHeightSave =
                        m_bNormalsSave = m_bInvalidNormalsSave =
                           m_bAvgWaveAngleAndHeightSave =
                              m_bWaveEnergySinceCollapseSave =
                                 m_bMeanWaveEnergySave = m_bBreakingWaveHeightSave =
                                    m_bPolygonBoundarySave = m_bCliffNotchSave =
                  ...
                  {
                     // Output only those vector files for which the user specified the
                     // code
                     ...
                     if (strRH.find(VECTOR_AVG_WAVE_ANGLE_AND_HEIGHT_CODE) !=
                         string::npos)
                     {
                     ...
                     if (strRH.find(VECTOR_WAVE_ENERGY_SINCE_COLLAPSE_CODE) !=
                         string::npos)
                     {
                        m_bWaveEnergySinceCollapseSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &VECTOR_WAVE_ENERGY_SINCE_COLLAPSE_CODE);
                     }
                     ...
                     if (strRH.find(VECTOR_BREAKING_WAVE_HEIGHT_CODE) !=
                         string::npos)
                     {
                        m_bBreakingWaveHeightSave = true;
                        strRH =
                           strRemoveSubstr(&strRH, &VECTOR_BREAKING_WAVE_HEIGHT_CODE);
                     }
                     ...
                     if (strRH.find(VECTOR_DOWNDRIFT_BOUNDARY_CODE) != string::npos)
                     {
                        ...
                        strRH =
                           strRemoveSubstr(&strRH, &VECTOR_DOWNDRIFT_BOUNDARY_CODE);
                     }

                     if (strRH.find(VECTOR_DEEP_WATER_WAVE_ANGLE_AND_HEIGHT_CODE) !=
                         string::npos)
                     {
                        m_bDeepWaterWaveAngleAndHeightSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &VECTOR_DEEP_WATER_WAVE_ANGLE_AND_HEIGHT_CODE);
                     }
                     ...
                     if (! strRH.empty())
                        strErr = "line " + to_string(nLine) + ": unknown code '" +
                           strRH + "' in list of vector GIS output codes";
                  }
               }
            ...
            case 18:
               ...
               if (! strRH.empty())
               {
                  ...
                  else
                  {
                     ...
                     if (strRH.find(TIME_SERIES_STILL_WATER_LEVEL_CODE) !=
                         string::npos)
                     {
                        m_bStillWaterLevelTSSave = true;
                        strRH = strRemoveSubstr(&strRH,
                     ...
                     if (strRH.find(TIME_SERIES_PLATFORM_EROSION_CODE) !=
                         string::npos)
                     {
                        m_bActualPlatformErosionTSSave = true;
                        strRH =
                           strRemoveSubstr(&strRH, &TIME_SERIES_PLATFORM_EROSION_CODE);
                     }

                     if (strRH.find(TIME_SERIES_CLIFF_COLLAPSE_EROSION_CODE) !=
                         string::npos)
                     {
                        m_bCliffCollapseErosionTSSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &TIME_SERIES_CLIFF_COLLAPSE_EROSION_CODE);
                     }

                     if (strRH.find(TIME_SERIES_CLIFF_COLLAPSE_DEPOSITION_CODE) !=
                         string::npos)
                     {
                        m_bCliffCollapseDepositionTSSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &TIME_SERIES_CLIFF_COLLAPSE_DEPOSITION_CODE);
                     }

                     if (strRH.find(TIME_SERIES_CLIFF_COLLAPSE_NET_CODE) !=
                         string::npos)
                     {
                        m_bCliffCollapseNetTSSave = true;
                        strRH = strRemoveSubstr(&strRH,
                     ...
                     if (strRH.find(TIME_SERIES_BEACH_EROSION_CODE) != string::npos)
                     {
                        m_bBeachErosionTSSave = true;
                        strRH =
                           strRemoveSubstr(&strRH, &TIME_SERIES_BEACH_EROSION_CODE);
                     }

                     if (strRH.find(TIME_SERIES_BEACH_DEPOSITION_CODE) !=
                         string::npos)
                     {
                        m_bBeachDepositionTSSave = true;
                        strRH =
                           strRemoveSubstr(&strRH, &TIME_SERIES_BEACH_DEPOSITION_CODE);
                     }

                     if (strRH.find(TIME_SERIES_BEACH_CHANGE_NET_CODE) !=
                         string::npos)
                     {
                        m_bBeachSedimentChangeNetTSSave = true;
                        strRH =
                           strRemoveSubstr(&strRH, &TIME_SERIES_BEACH_CHANGE_NET_CODE);
                     }

                     if (strRH.find(TIME_SERIES_SUSPENDED_SEDIMENT_CODE) !=
                         string::npos)
                     {
                        m_bSuspSedTSSave = true;
                        strRH = strRemoveSubstr(&strRH,
                     ...
                     if (strRH.find(TIME_SERIES_FLOOD_SETUP_SURGE_CODE) !=
                         string::npos)
                     {
                        m_bFloodSetupSurgeTSSave = true;
                        strRH = strRemoveSubstr(&strRH,
                     ...
                     if (strRH.find(TIME_SERIES_FLOOD_SETUP_SURGE_RUNUP_CODE) !=
                         string::npos)
                     {
                        m_bFloodSetupSurgeRunupTSSave = true;
                        strRH = strRemoveSubstr(
                           &strRH, &TIME_SERIES_FLOOD_SETUP_SURGE_RUNUP_CODE);
                     }

                     // Check to see if all codes have been removed
                     if (! strRH.empty())
                        strErr = "line " + to_string(nLine) + ": unknown code '" +
                           strRH + "' in list of time series output files";
                  }
               }
            ...
            case 19:
               // Vector coastline smoothing algorithm: 0 = none, 1 = running mean, 2
               // = Savitzky-Golay
               if (! bIsStringValidInt(strRH))
               {
                  strErr = "line " + to_string(nLine) +
                     ": invalid integer for coastline smoothing algorithm '" +
                     strRH + "' in " + m_strDataPathName;
                  break;
               }
               ...
               if ((m_nCoastSmooth < SMOOTH_NONE) ||
                   (m_nCoastSmooth > SMOOTH_SAVITZKY_GOLAY))
                  strErr = "line " + to_string(nLine) +
            ...
            case 20:
               ...
               {
                  strErr = "line " + to_string(nLine) +
                     ": invalid integer for coastline smoothing window '" +
                     strRH + "' in " + m_strDataPathName;
                  break;
               }
               ...
               if ((m_nCoastSmoothingWindowSize <= 0) ||
                   ! (m_nCoastSmoothingWindowSize % 2))
                  strErr = "line " + to_string(nLine) +
                     ": size of coastline vector smoothing window (must be > 0 "
                     "and odd)";

               break;

            case 21:
               // Order of coastline profile smoothing polynomial for Savitzky-Golay:
               // usually 2 or 4, max is 6
               if (! bIsStringValidInt(strRH))
               {
                  strErr = "line " + to_string(nLine) +
                     ": invalid integer for Savitzky-Golay polynomial for "
                     "coastline smoothing '" +
                     strRH + "' in " + m_strDataPathName;
                  break;
               }
               ...
               if ((m_nSavGolCoastPoly <= 0) || (m_nSavGolCoastPoly > 6))
                  strErr = "line " + to_string(nLine) +
                     ": value of Savitzky-Golay polynomial for coastline "
                     "smoothing (must be <= 6)";
            ...
            case 23:
               ...
               if (! bIsStringValidInt(strRH))
               {
                  strErr =
                     "line " + to_string(nLine) +
                     ": invalid integer for size of coastline smoothing window '" +
                     strRH + "' in " + m_strDataPathName;
                  break;
               }
               ...
               if ((m_nProfileSmoothWindow < 0) ||
                   (m_nProfileSmoothWindow > 0 && ! (m_nProfileSmoothWindow % 2)))
                  strErr = "line " + to_string(nLine) +
                     ": size of profile vector smoothing window (must be >= 0, "
                     "if > 0 must be odd)";
            ...
            case 24:
               ...
               {
                  strErr = "line " + to_string(nLine) +
                     ": invalid floating point number for max local slope '" +
                     strRH + "' in " + m_strDataPathName;
                  break;
               }
               ...
               if (m_dProfileMaxSlope <= 0)
                  strErr =
                     "line " + to_string(nLine) + ": max local slope must be > 0";

               break;

            case 25:
               // Maximum elevation of beach above SWL, first check that this is a
               // valid double
               if (! bIsStringValidDouble(strRH))
               {
                  strErr = "line " + to_string(nLine) +
                     ": invalid floating point number for maximum elevation of "
                     "beach above SWL '" +
                     strRH + "' in " + m_strDataPathName;
                  break;
               }
            ...
            case 26:
               ...
               {
                  strErr = "line " + to_string(nLine) +
                     ": invalid integer for number of sediment layers '" +
                     strRH + "' in " + m_strDataPathName;
                  break;
               }
                  ...
                  for (int j = 1; j <= 6; j++)
                  {
                     if (! bFirst)
                     {
                        do
                        {
                           ...
                           {
                              cerr << ERR << "premature end of file in "
                        ...
                        // If it is a blank line or a comment then ignore it
                        while (strRec.empty() || (strRec[0] == QUOTE1) ||
                               (strRec[0] == QUOTE2));

                        // Not blank or a comment, so find the colon: lines MUST have a
                        // colon separating data from leading description portion
                        nPos = strRec.find(COLON);
                        ...
                        if (nPos == string::npos)
                        {
                           // Error: badly formatted (no colon)
                           cerr << ERR << "on line " << to_string(nLine)
                                << ": badly formatted (no ':') in " << m_strDataPathName
                                << endl
                                << strRec << endl;
                           return false;
                        }

                        // Strip off leading portion (the bit up to and including the
                        // colon)
                        ...
                        // Look for a trailing comment, if found then terminate string
                        // at that point and trim off any trailing whitespace
                        nPos = strRH.rfind(QUOTE1);
...
#ifdef _WIN32
                        // For Windows, make sure has backslashes, not Unix-style
                        // slashes
                        strRH = pstrChangeToBackslash(&strRH);
#endif
                     }
                     ...
                     switch (j)
                     {
                        case 1:
                           ...
                           // Initial fine unconsolidated sediment depth GIS file (can be
                           // blank)
                           if (! strRH.empty())
                           {
                              // Set switch
                              ...
                              // Now check for leading slash, or leading Unix home dir
                              // symbol, or occurrence of a drive letter
                              if ((strRH[0] == PATH_SEPARATOR) || (strRH[0] == TILDE) ||
                                  (strRH[1] == COLON))
                              {
                                 // It has an absolute path, so use it 'as is'
                        ...
                        case 2:
                           // Initial sand unconsolidated sediment depth GIS file (can be
                           // blank)
                           if (! strRH.empty())
                           {
                              // Set switch
                        ...
                        case 3:
                           // Initial coarse unconsolidated sediment depth GIS file (can be
                           // blank)
                           if (! strRH.empty())
                           {
                              // Set switch
                        ...
                        case 4:
                           // Initial fine consolidated sediment depth GIS file (can be
                           // blank)
                           if (! strRH.empty())
                           {
                              // Set switches
                        ...
                        case 5:
                           // Initial sand consolidated sediment depth GIS file (can be
                           // blank)
                           if (! strRH.empty())
                           {
                              // Set switches
                        ...
                        case 6:
                           // Initial coarse consolidated sediment depth GIS file (can be
                           // blank)
                           if (! strRH.empty())
                           {
                              // Set switches
            ...
            case 31:
               // Initial Intervention class GIS file (can be blank: if so then
               // intervention height file must also be blank)
               if (! strRH.empty())
               {
#ifdef _WIN32
                  // For Windows, make sure has backslashes, not Unix-style slashes
            ...
            case 32:
               // Initial Intervention height GIS file (can be blank: if so then
               // intervention class file must also be blank)
               if (strRH.empty())
               {
                  if (! m_strInterventionClassFile.empty())
                     strErr = "line " + to_string(nLine) +
                        ": must specify both intervention class and "
                        "intervention height files";
            ...
            case 33:
               ...
               if ((m_nWavePropagationModel != WAVE_MODEL_COVE) &&
                   (m_nWavePropagationModel != WAVE_MODEL_CSHORE))
                  strErr = "line " + to_string(nLine) +
            ...
            case 34:
               // Density of sea water (kg/m3), first check that this is a valid
               // double
               if (! bIsStringValidDouble(strRH))
               {
                  strErr = "line " + to_string(nLine) +
                     ": invalid floating point number for sea water density '" +
                     strRH + "' in " + m_strDataPathName;
                  break;
               }
               ...
               if (m_dSeaWaterDensity <= 0)
                  strErr =
                     "line " + to_string(nLine) + ": sea water density must be > 0";

               break;

            case 35:
               // Initial mean still water level (m), first check that this is a
               // valid double TODO 041 Make this a per-timestep SWL file
               if (! bIsStringValidDouble(strRH))
               {
                  strErr = "line " + to_string(nLine) +
                     ": invalid floating point number for initial SWL '" +
                     strRH + "' in " + m_strDataPathName;
                  break;
               }
            ...
            case 36:
               ...
               else
               {
                  ...
                  {
                     strErr = "line " + to_string(nLine) +
                        ": invalid floating point number for final SWL '" +
                        strRH + "' in " + m_strDataPathName;
                     break;
                  }
            ...
            case 37:
               // Deep water wave height (m) or a file of point vectors giving deep
               // water wave height (m) and orientation (for units, see below)
               if (strRH.empty())
                  strErr = "line " + to_string(nLine) +
                     ": deep water wave height in " + m_strDataPathName +
                     " must be either a number or a filename (filename must "
                     "not start with a number)";

               else
               {
                  if (isdigit(
                         strRH.at(0)))      // If this starts with a number then is a
                                            // single value, otherwise is a filename. Note
                                            // that filename must not start with number
                  {
                     // Just one value of wave height for all deep water cells, first
                     // check that this is a valid double
                     if (! bIsStringValidDouble(strRH))
                     {
                        strErr = "line " + to_string(nLine) +
                           ": invalid floating point number for deep water wave "
                           "height '" +
                           strRH + "' in " + m_strDataPathName;
                        break;
                     }
                  ...
                  {
                     // We are reading deep water wave height and deep water wave
                     // orientation from two files. This first file is a point shape
                     // file with the location of the buoys and integer ID for each one
                     ...
                     // Now check for leading slash, or leading Unix home dir symbol,
                     // or occurrence of a drive letter
                     if ((strRH[0] == PATH_SEPARATOR) || (strRH[0] == TILDE) ||
                         (strRH[1] == COLON))
                     {
                        // It has an absolute path, so use it 'as is'
            ...
            case 38:
               // Deep water wave height input file. Each point in
               // m_strDeepWaterWavesInputFile is a triad of wave height, orientation
               // and period for each time step
            ...
            case 39:
               // Deep water wave orientation in input CRS: this is the oceanographic
               // convention i.e. direction TOWARDS which the waves move (in degrees
               // clockwise from north)
               if (! m_bHaveWaveStationData)
               {
                  // Only read this if we have just a single value of wave height for
                  // all deep water cells. Check that this is a valid double
                  if (! bIsStringValidDouble(strRH))
                  {
                     strErr = "line " + to_string(nLine) +
                        ": invalid floating point number for deep water wave "
            ...
            case 40:
               ...
               if (! m_bHaveWaveStationData)
               {
                  // Only read this if we also have just a single value of wave height
                  // for all deep water cells. Check that this is a valid double
                  if (! bIsStringValidDouble(strRH))
                  {
                     strErr = "line " + to_string(nLine) +
                        ": invalid floating point number for wave period '" +
                        strRH + "' in " + m_strDataPathName;
                     break;
                  }
            ...
            case 41:
               // Tide data file (can be blank). This is the change (m) from still
            ...
            case 42:
               // Breaking wave height-to-depth ratio, check that this is a valid
               // double
               if (! bIsStringValidDouble(strRH))
               {
                  strErr = "line " + to_string(nLine) +
                     ": invalid floating point number for breaking wave height "
                     "to depth ratio '" +
                     strRH + "' in " + m_strDataPathName;
                  break;
               }
            ...
            case 44:
               // If simulating coast platform erosion, R (coast platform resistance
               ...
               if (m_bDoShorePlatformErosion)
               {
                  ...
                  {
                     strErr = "line " + to_string(nLine) +
                        ": invalid floating point number for R (coast platform "
                  ...
                  if (m_dR <= 0)
                     strErr = "line " + to_string(nLine) +
                        ": R (coast platform resistance to erosion) value must "
                        "be > 0";
               }
            ...
            case 46:
               // If simulating beach sediment transport, beach sediment transport at
               // grid edges [0 = closed, 1 = open, 2 = re-circulate]
               if (m_bDoBeachSedimentTransport)
               {
                  if (! bIsStringValidInt(strRH))
                  {
                     strErr = "line " + to_string(nLine) +
                        ": invalid integer for beach sediment transport at grid "
                        "edges '" +
                        strRH + "' in " + m_strDataPathName;
                     break;
                  }
                  ...
                  if ((m_nUnconsSedimentHandlingAtGridEdges < GRID_EDGE_CLOSED) ||
                      (m_nUnconsSedimentHandlingAtGridEdges > GRID_EDGE_RECIRCULATE))
                     strErr = "line " + to_string(nLine) +
                        ": switch for handling of beach sediment at grid edges "
                        "must be 0, 1, or 2";
               }
            ...
            case 47:
               // If simulating beach sediment transport, beach erosion/deposition
               ...
               if (m_bDoBeachSedimentTransport)
               {
                  if (! bIsStringValidInt(strRH))
                  {
                     strErr =
                        "line " + to_string(nLine) +
                        ": invalid integer for beach erosion/deposition equation '" +
                        strRH + "' in " + m_strDataPathName;
                     break;
                  }
            ...
            case 48:
               // Median size of fine sediment (mm), always needed [0 = default, only
               // for Kamphuis eqn]. First check that this is a valid double
               if (! bIsStringValidDouble(strRH))
               {
                  strErr = "line " + to_string(nLine) +
                     ": invalid floating point number for median particle size "
                     "of fine sediment '" +
                     strRH + "' in " + m_strDataPathName;
                  break;
               }
            ...
            case 49:
               // Median size of sand sediment (mm), always needed [0 = default, only
               // for Kamphuis eqn]. First check that this is a valid double
               if (! bIsStringValidDouble(strRH))
               {
                  strErr = "line " + to_string(nLine) +
            ...
            case 50:
               // Median size of coarse sediment (mm), always needed [0 = default,
            ...
            case 51:
               ...
               if (m_bDoBeachSedimentTransport)
               {
                  ...
                  {
                     strErr = "line " + to_string(nLine) +
                        ": invalid floating point number for density of beach "
                        "sediment '" +
                        strRH + "' in " + m_strDataPathName;
                     break;
                  }
            ...
            case 52:
               ...
               if (m_bDoBeachSedimentTransport)
               {
                  ...
                  {
                     strErr = "line " + to_string(nLine) +
                        ": invalid floating point number for porosity of beach "
                        "sediment '" +
                        strRH + "' in " + m_strDataPathName;
                     break;
                  }
            ...
            case 53:
               // Relative erodibility (0 - 1) of fine-sized sediment, always needed.
               // First check that this is a valid double
               if (! bIsStringValidDouble(strRH))
               {
                  strErr = "line " + to_string(nLine) +
                     ": invalid floating point number for erodibility of "
                     "fine-sized sediment '" +
                     strRH + "' in " + m_strDataPathName;
                  break;
               }
               ...
               if ((m_dFineErodibility < 0) || (m_dFineErodibility > 1))
                  strErr = "line " + to_string(nLine) +
                     ": relative erodibility of fine-sized sediment must be "
                     "between 0 and 1";

               break;

            case 54:
               // Relative erodibility (0 - 1) of sand-sized sediment, always needed.
               // First check that this is a valid double
               if (! bIsStringValidDouble(strRH))
               {
                  strErr = "line " + to_string(nLine) +
               ...
               if ((m_dSandErodibility < 0) || (m_dSandErodibility > 1))
                  strErr = "line " + to_string(nLine) +
                     ": relative erodibility of sand-sized sediment must be "
                     "between 0 and 1";

               break;

            case 55:
               // Relative erodibility (0 - 1) of coarse-sized sediment, always
               // needed. First check that this is a valid double
               if (! bIsStringValidDouble(strRH))
               {
                  strErr = "line " + to_string(nLine) +
                     ": invalid floating point number for erodibility of "
                     "coarse-sized sediment '" +
                     strRH + "' in " + m_strDataPathName;
                  break;
               }
               ...
               {
                  strErr = "line " + to_string(nLine) +
                     ": relative erodibility of coarse-sized sediment must be "
                     "between 0 and 1";
                  break;
               }

               if ((m_dFineErodibility + m_dSandErodibility +
            ...
            case 56:
               ...
               if (m_bDoBeachSedimentTransport)
               {
                  ...
                  {
                     strErr = "line " + to_string(nLine) +
                        ": invalid floating point number for transport "
                        "parameter KLS of CERC equation '" +
                        strRH + "' in " + m_strDataPathName;
                     break;
                  }
                  ...
                  // However, many sites do not have transport data available to
                  // calibrate K, and for design applications without calibration data
                  // the CERC formula provides only order-of-magnitude accuracy
                  // (Fowler et al., 1995; Wang et al., 1998). The recommended value
                  // of K = 0.39 has been commonly used to represent the potential
                  // longshore transport rate. However, Miller (1998) found that the
                  // CERC formula sometimes over and sometimes under predicted
                  // longshore transport rate for measurements during storms,
                  // indicating the value of K also can be higher than 0.39
                  // TODO 042 Should this be a user input, or not? The comment above
                  // seems inconclusive m_dKLS = tMin(m_dKLS, 0.39);
            ...
            case 57:
               ...
               if (m_bDoBeachSedimentTransport)
               {
                  ...
                  if (! bIsStringValidDouble(strRH))
                  {
                     strErr = "line " + to_string(nLine) +
                        ": invalid floating point number for transport "
                        "parameter of Kamphuis equation '" +
                        strRH + "' in " + m_strDataPathName;
                     break;
                  }
            ...
            case 58:
               // Berm height i.e. height above SWL of start of depositional Dean
               // profile
               if (m_bDoBeachSedimentTransport)
               {
                  // First check that this is a valid double
                  ...
                  {
                     strErr = "line " + to_string(nLine) +
                        ": invalid floating point number for Dean profile start "
                        "height above SWL '" +
                        strRH + "' in " + m_strDataPathName;
                     break;
                  }
                  ...
                  if (m_dDeanProfileStartAboveSWL < 0)
                     strErr = "line " + to_string(nLine) +
                        ": Berm height (Dean profile start height above SWL) "
                        "must be >= 0";
               }
            ...
            case 59:
               ...
               {
                  // Only consider cliff collapse if we have some consolidated
                  // sedimemt
                  strRH = strToLower(&strRH);
            ...
            case 60:
               ...
               if (m_bHaveConsolidatedSediment && m_bDoCliffCollapse)
               {
                  ...
                  {
                     strErr = "line " + to_string(nLine) +
                        ": invalid floating point number for cliff resistance "
                        "to erosion '" +
                        strRH + "' in " + m_strDataPathName;
                     break;
                  }
            ...
            case 61:
               ...
               if (m_bHaveConsolidatedSediment && m_bDoCliffCollapse)
               {
                  ...
                  {
                     strErr = "line " + to_string(nLine) +
                        ": invalid floating point number for cliff notch "
                        "overhang at collapse '" +
                        strRH + "' in " + m_strDataPathName;
                     break;
                  }
            ...
            case 63:
               ...
               if (m_bHaveConsolidatedSediment && m_bDoCliffCollapse)
               {
                  ...
                  {
                     strErr = "line " + to_string(nLine) +
                        ": invalid floating point number for scale parameter A "
                        "for cliff deposition '" +
                        strRH + "' in " + m_strDataPathName;
                     break;
                  }
                  ...
                  if (m_dCliffDepositionA < 0)
                     strErr = "line " + to_string(nLine) +
                        ": scale parameter A for cliff deposition must be 0 [= "
                        "auto] or greater";
               }
            ...
            case 64:
               ...
               if (m_bHaveConsolidatedSediment && m_bDoCliffCollapse)
               {
                  ...
                  {
                     strErr = "line " + to_string(nLine) +
                        ": invalid floating point number for width of cliff "
                        "collapse talus '" +
                        strRH + "' in " + m_strDataPathName;
                     break;
                  }
            ...
            case 65:
               ...
               if (m_bHaveConsolidatedSediment && m_bDoCliffCollapse)
               {
                  ...
                  {
                     strErr = "line " + to_string(nLine) +
                        ": invalid floating point number for planview length of "
                        "cliff deposition '" +
                        strRH + "' in " + m_strDataPathName;
                     break;
                  }
            ...
            case 66:
               // Minimum height of landward end of talus, as a fraction of cliff
               // elevation
               if (m_bHaveConsolidatedSediment && m_bDoCliffCollapse)
               {
                  // First check that this is a valid double
                  ...
                  {
                     strErr = "line " + to_string(nLine) +
                        ": invalid floating point number for height of cliff "
                        "collapse (as a fraction of cliff elevation) '" +
                        strRH + "' in " + m_strDataPathName;
                     break;
                  }
                  ...
                  if (m_dMinCliffTalusHeightFrac < 0)
                     strErr = "line " + to_string(nLine) +
                        ": minimum height of cliff collapse (as a fraction of "
                        "cliff elevation) must be >= 0";
               }
            ...
            case 68:
               ...
               if (m_bRiverineFlooding)
               {
                  if (! strRH.empty())
                  {
                     ...
                     // First look for "all"
                     if (strRH.find(VECTOR_ALL_RIVER_FLOOD_OUTPUT_CODE) !=
                         string::npos)
                     {
                        m_bFloodSWLSetupLine = m_bFloodSWLSetupSurgeLine =
                           m_bFloodSWLSetupSurgeRunupLine = true;
                     }

                     else
                     {
                        // We are not outputting all vector flood GIS files, so set
                        // switches (and remove strings) for those optional files for
                        // which the user specified the code
                        if (strRH.find(VECTOR_FLOOD_SWL_SETUP_LINE_CODE) !=
                            string::npos)
                        {
                        ...
                        if (strRH.find(VECTOR_FLOOD_SWL_SETUP_SURGE_LINE_CODE) !=
                            string::npos)
                        {
                           m_bFloodSWLSetupSurgeLine = true;
                           strRH = strRemoveSubstr(
                              &strRH, &VECTOR_FLOOD_SWL_SETUP_SURGE_LINE_CODE);
                        }

                        if (strRH.find(VECTOR_FLOOD_SWL_SETUP_SURGE_RUNUP_LINE_CODE) !=
                            string::npos)
                        {
                           m_bFloodSWLSetupSurgeRunupLine = true;
                           strRH = strRemoveSubstr(
                              &strRH, &VECTOR_FLOOD_SWL_SETUP_SURGE_RUNUP_LINE_CODE);
                        }
                     ...
                     strErr = "line " + to_string(nLine) +
                        ...
                        "' or at least one vector GIS output code for riverine "
                        "flooding";
               }
            ...
            case 69:
               if (m_bRiverineFlooding)
               {
                  ...
                  if (bIsStringValidInt(strRH))
                  {
                     ...
                     strErr = "line " + to_string(nLine) +
                        ": invalid code for run-up equation used in simulating "
                        "floods";
               }
            ...
            case 71:
               if (m_bRiverineFlooding)
               {
                  ...
                  if (! strRH.empty())
                  {
                     ...
                     strErr = "line " + to_string(nLine) +
                        ": path of location points file must not be empty if "
                        "simulating floods";
               }
            ...
            case 74:
               // Sediment input type: required if have shapefile [P = Point, C =
               // coast block, L = line]
               if (m_bSedimentInput)
               {
                  strRH = strToLower(&strRH);
            ...
            // --------------------------------------------------
            case 76:
               // Gravitational acceleration (m2/s). First check that this is a valid
               // double
               if (! bIsStringValidDouble(strRH))
               {
                  strErr = "line " + to_string(nLine) +
                     ": invalid floating point number for gravitational "
                     "acceleration '" +
                     strRH + "' in " + m_strDataPathName;
                  break;
               }
            ...
            case 78:
               ...
               {
                  strErr = "line " + to_string(nLine) +
                     ": invalid floating point number for random factor for "
                     "spacing of coastline normals '" +
                     strRH + "' in " + m_strDataPathName;
                  break;
               }
            ...
            case 79:
               ...
               {
                  strErr = "line " + to_string(nLine) +
                     ": invalid floating point number for length of coastline "
                     "normals '" +
                     strRH + "' in " + m_strDataPathName;
                  break;
               }
            ...
            case 80:
               // Start depth for wave calcs (ratio to deep water wave height), check
               // that this is a valid double
               if (! bIsStringValidDouble(strRH))
               {
                  strErr = "line " + to_string(nLine) +
                     ": invalid floating point number for start depth for wave "
                     "calcs '" +
                     strRH + "' in " + m_strDataPathName;
                  break;
               }
            ...
            case 81:
               ...
               if (strRH.find('y') != string::npos)
               {
                  ...
                  {
                     strErr = "line " + to_string(nLine) +
                        ": cannot save profiile data if not simulating shore "
                        "platform erosion";
                     break;
                  }

                  // TODO 043 What about randomness of profile spacing, since profile
                  // location is determined by curvature?
               }
            ...
            case 82:
               ...
               if (m_bOutputProfileData)
               {
                  ...
                  for (unsigned int j = 0; j < VstrTmp.size(); j++)
                  {
                     ...
                     {
                        strErr = "line " + to_string(nLine) +
                           ": invalid integer for profile to be saved '" +
                           VstrTmp[j] + "' in " + m_strDataPathName;
                        break;
                     }
            ...
            case 86:
               // Size of moving window for coastline curvature calculation (must be
               // odd)
               if (! bIsStringValidInt(strRH))
               {
                  strErr = "line ";
                  strErr += to_string(nLine);
                  strErr +=
                     ": invalid integer for size of moving window for "
                     "coastline curvature calculation '";
                  strErr += strRH;
                  strErr += "' in ";
               ...
               if ((m_nCoastCurvatureMovingWindowSize <= 0) ||
                   ! (m_nCoastCurvatureMovingWindowSize % 2))
               {
                  strErr = "line ";
                  strErr += to_string(nLine);
                  strErr +=
                     ": size of moving window for coastline curvature "
                     "calculation (must be > 0 and odd)";
               }

               break;

            case 87:
               // Cliff edge smoothing algorithm: 0 = none, 1 = running mean, 2 =
               // Savitzky-Golay
               if (! bIsStringValidInt(strRH))
               {
                  strErr = "line ";
               ...
               if ((m_nCliffEdgeSmooth < SMOOTH_NONE) ||
                  ...
                  strErr =
                     "line " + to_string(nLine) + ": cliff edge smoothing algorithm";
            ...
            case 88:
               ...
               if ((m_nCliffEdgeSmoothWindow <= 0) ||
                   ! (m_nCliffEdgeSmoothWindow % 2))
                  strErr =
                     "line " + to_string(nLine) +
            ...
            case 89:
               // Order of cliff edge smoothing polynomial for Savitzky-Golay:
               // usually 2 or 4, max is 6
               if (! bIsStringValidInt(strRH))
               {
                  strErr = "line " + to_string(nLine) +
                     ": invalid integer for Savitzky-Golay polynomial for "
                     "cliff edge smoothing '" +
                     strRH + "' in " + m_strDataPathName;
                  break;
               }
               ...
               if ((m_nSavGolCliffEdgePoly < 2) || (m_nSavGolCliffEdgePoly > 6) ||
                  ...
                  strErr = "line " + to_string(nLine) +
                     ": order of Savitzky-Golay polynomial for cliff edge "
                     "smoothing (must be 2, 4 or 6)";
            ...
            case 90:
               ...
               if (m_dCliffSlopeLimit <= 0)
                  strErr =
                     "line " + to_string(nLine) + ": cliff slope limit must be > 0";
   ...
   // Finally, need to check that we have at least one raster file, so that we
   // know the grid size and units (and preferably also the projection)
   bool bNoRasterFiles = true;

   if ((! m_strInitialBasementDEMFile.empty()) ||
       (! m_strInitialSuspSedimentFile.empty()) ||
       (! m_strInitialLandformFile.empty()) ||
       (! m_strInterventionHeightFile.empty()))
      bNoRasterFiles = false;

   for (int j = 0; j < m_nLayers; j++)
   {
      if ((! m_VstrInitialFineUnconsSedimentFile[j].empty()) ||
          (! m_VstrInitialSandUnconsSedimentFile[j].empty()) ||
          (! m_VstrInitialCoarseUnconsSedimentFile[j].empty()) ||
          (! m_VstrInitialFineConsSedimentFile[j].empty()) ||
          (! m_VstrInitialSandConsSedimentFile[j].empty()) ||
          (! m_VstrInitialCoarseConsSedimentFile[j].empty()))
         bNoRasterFiles = false;
   }
...
int CSimulation::nReadTideDataFile()
{
   ...
   while (getline(InStream, strRec))
   {
      ...
      {
         cerr << ERR << "invalid floating point number for tide data '" << strRec
              << "' on line " << nLine << " of " << m_strTideDataFile << endl;
         return RTN_ERR_TIDEDATAFILE;
      }
...
int CSimulation::nReadShapeFunctionFile()
{
   ...
   if (! InStream.is_open())
   {
      ...
      cerr << ERR << "cannot open " << m_strSCAPEShapeFunctionFile << " for input"
           << endl;
   ...
   // Now read the rest of the data from the file to get the Erosion Potential
   // Shape function
   while (getline(InStream, strRec))
   {
      nLine++;
         ...
         if (! bIsStringValidDouble(strTmp[i]))
         {
            cerr << ERR
                 << "on line " + to_string(nLine) +
                  "  invalid floating point number for Erosion Potential "
                  "Shape data '"
                 << strTmp[i] << "' in " << m_strSCAPEShapeFunctionFile << endl;
            return RTN_ERR_SCAPE_SHAPE_FUNCTION_FILE;
         }
      }
   ...
   if (nExpected != nRead)
   {
      cout << ERR << "read in " << nRead << " lines from "
           << m_strSCAPEShapeFunctionFile << " but " << nExpected
           << " lines expected" << endl;
      return RTN_ERR_SCAPE_SHAPE_FUNCTION_FILE;
   }

   // Is the shape funcion well defined? i.e. it must be -ve or 0.0 for all
   // values
   for (unsigned int j = 0; j < m_VdErosionPotential.size(); j++)
   {
      if (m_VdErosionPotential[j] > 0)
      {
         cout << ERR << " in " << m_strSCAPEShapeFunctionFile
              << ", erosion potential function cannot be positive" << endl;
         return RTN_ERR_SCAPE_SHAPE_FUNCTION_FILE;
      }
   }

   // OK, now use this data to create a look-up table to be used for the rest of
   // the simulation
   if (! bCreateErosionPotentialLookUp(&VdDepthOverDB, &VdErosionPotential,
                                       &VdErosionPotentialFirstDeriv))
   {
      cout << ERR << "line " + to_string(nLine) + "  in "
...
int CSimulation::nReadWaveStationInputFile(int const nWaveStations)
{
   ...
   if (! InStream.is_open())
   {
      ...
      cerr << ERR << "cannot open " << m_strDeepWaterWavesInputFile
           << " for input" << endl;
   ...
   while (getline(InStream, strRec))
   {
      ...
      if ((! strRec.empty()) && (strRec[0] != QUOTE1) && (strRec[0] != QUOTE2))
      {
         ...
         // The header lines (the first four lines of the file) contains leading
         // description separated by a colon from the data
         if (nRead < 5)
         {
            // Find the colon: note that lines MUST have a colon separating data
            // from leading description portion
            ...
            if (nPos == string::npos)
            {
               // Error: badly formatted (no colon)
               cerr << ERR << "on line " << to_string(nLine)
                    << "badly formatted (no ':') in " << m_strDeepWaterWavesInputFile
                    << endl
                    << "'" << strRec << "'" << endl;
            ...
            if (nPos == strRec.size() - 1)
            {
               ...
               cerr << ERR << "on line " << to_string(nLine)
                    << "badly formatted (nothing following ':') in "
                    << m_strDeepWaterWavesInputFile << endl
                    << "'" << strRec << "'" << endl;
               return RTN_ERR_READING_DEEP_WATER_WAVE_DATA;
            }
            ...
            {
               case 1:
                  // Get the start date/time for this data, format is [hh-mm-ss
                  // dd/mm/yyyy]
                  VstrTmp = VstrSplit(&strRH, SPACE);
                  ...
                  if ((nSec != m_nSimStartSec) || (nMin != m_nSimStartMin) ||
                      (nHour != m_nSimStartHour) || (nDay != m_nSimStartDay) ||
                      (nMonth != m_nSimStartMonth) || (nYear != m_nSimStartYear))
                  {
                     strErr = "line " + to_string(nLine) +
                        ": start time and date for wave time series data differs "
                        "from simulation start time and date,";
                     break;
                  }
               ...
               case 2:
                  ...
                  {
                     strErr =
                        "line " + to_string(nLine) + ": unknown units for timestep";
                     break;
                  }
               ...
               case 3:
                  ...
                  if (! bIsStringValidInt(strRH))
                  {
                     strErr = "line " + to_string(nLine) +
                        ...
                        "' in " + m_strDeepWaterWavesInputFile;
                     break;
                  }

                  nExpectedStations = stoi(strRH);

                  // Check that the number of expected stations is equal to the number
                  // of stations on the point shape file
                  if (nExpectedStations != nWaveStations)
                  {
                     // Error: number of points on shape file does not match the number
                     // of stations on the wave time series file
                     strErr = "line " + to_string(nLine) +
                        ": number of wave stations in " +
                        m_strDeepWaterWaveStationsShapefile + " is " +
                        to_string(nWaveStations) + " but we have " +
                        to_string(nExpectedStations) + " stations";
               ...
               case 4:
                  ...
                  {
                     strErr = "line " + to_string(nLine) +
                        ": invalid integer for expected number of time steps '" +
                        strRH + "' in " + m_strDeepWaterWaveStationsShapefile;
                     break;
                  }
         ...
         else
         {
            ...
            // Read in each wave attribute for each time step and station: split the
            // string, and remove whitespace
            vector<string> VstrTmp = VstrSplit(&strRec, COMMA);

            for (unsigned int i = 0; i < VstrTmp.size();
                 i++)      // VstrTmp.size() should be 3 x nExpectedStations
            {
               // Remove leading and trailing whitespace
               ...
               if (! bIsStringValidDouble(VstrTmp[i]))
               {
                  strErr =
                     "line " + to_string(nLine) +
                     ": invalid floating point number for deep water wave value '" +
                     VstrTmp[i] + "' in " + m_strDeepWaterWavesInputFile;
                  break;
               }
            }
            ...
            {
               m_VdTSDeepWaterWaveStationHeight.push_back(
                  strtod(VstrTmp[n + 0].c_str(), NULL));
               m_VdTSDeepWaterWaveStationAngle.push_back(
                  strtod(VstrTmp[n + 1].c_str(), NULL));
               m_VdTSDeepWaterWaveStationPeriod.push_back(
                  strtod(VstrTmp[n + 2].c_str(), NULL));

               // Check some simple wave input stats
               if (m_VdTSDeepWaterWaveStationHeight.back() >
                   m_dMaxUserInputWaveHeight)
                  m_dMaxUserInputWaveHeight = m_VdTSDeepWaterWaveStationHeight.back();

               if (m_VdTSDeepWaterWaveStationPeriod.back() >
      ...
      {
         // Error in input to initialisation file
         cerr << ERR << strErr << " in deep water wave time series file "
              << m_strDeepWaterWavesInputFile << endl
              << "'" << strRec << "'" << endl;
         InStream.close();
   ...
   {
      // Error: number of timesteps read does not match the number given in the
      // file's header
      cerr << ERR << "in " << m_strDeepWaterWavesInputFile << ", data for "
           << nTimeStepsRead << " timesteps was read, but "
   ...
   // Did we read in what we expected?
   unsigned int const nTotExpected =
      nExpectedStations * m_nDeepWaterWaveDataNumTimeSteps;

   if (m_VdTSDeepWaterWaveStationHeight.size() != nTotExpected)
   {
      cout << ERR << "read in " << m_VdTSDeepWaterWaveStationHeight.size()
           << " lines from " << m_strDeepWaterWavesInputFile << " but "
           << nTotExpected << " values expected" << endl;
   ...
   {
      cout << ERR << "read in " << m_VdTSDeepWaterWaveStationAngle.size()
           << " lines from " << m_strDeepWaterWavesInputFile << " but "
           << nTotExpected << " values expected" << endl;
   ...
   {
      cout << ERR << "read in " << m_VdTSDeepWaterWaveStationPeriod.size()
           << " lines from " << m_strDeepWaterWavesInputFile << " but "
           << nTotExpected << " values expected" << endl;
   ...
   // All is OK, so we can now initialize the vectors that will store this
   // timestep's deep water wave values
   ...
   // Finally, check whether the wave data will 'wrap' i.e. whether the number of
   // timesteps is less than the total number of timesteps in the simulation
   int const nSimulationTimeSteps =
      static_cast<int>(floor(m_dSimDuration / m_dTimeStep));
   ...
   if (m_nDeepWaterWaveDataNumTimeSteps < nSimulationTimeSteps)
   {
      ...
      string const strTmp =
         "Deep water wave data will wrap every " +
         (m_nDeepWaterWaveDataNumTimeSteps > 1 ? to_string(m_nDeepWaterWaveDataNumTimeSteps) + " " : "") +
         "time step" + (m_nDeepWaterWaveDataNumTimeSteps > 1 ? "s" : "") +
         " (every " + to_string(m_dWaveDataWrapHours) + " hours)\n";
...
int CSimulation::nReadSedimentInputEventFile(void)
{
   ...
   {
      // Error: cannot open time series file for input
      cerr << ERR << "cannot open " << m_strSedimentInputEventFile << " for input"
           << endl;
      ...
      if ((! strRec.empty()) && (strRec[0] != QUOTE1) && (strRec[0] != QUOTE2))
      {
         ...
         if (VstrTmp.size() < nTarget)
         {
            strErr = "line " + to_string(nLine) +
               ...
               "' in " + m_strSedimentInputEventFile;
            break;
         }

         // First item is the Location ID of the sediment input event (same as the
         // ID in the shapefile)
         if (! bIsStringValidInt(VstrTmp[0]))
         {
            strErr = "line " + to_string(nLine) +
               ": invalid integer for Location ID of sediment input event '" +
               VstrTmp[0] + "' in " + m_strSedimentInputEventFile;
            break;
         }
         ...
         auto result = find(m_VnSedimentInputLocationID.begin(),
                            m_VnSedimentInputLocationID.end(), nID);

         if (result == m_VnSedimentInputLocationID.end())
         {
            strErr = "line " + to_string(nLine) + ": invalid Location ID '" +
               to_string(nID) +
         ...
         // Next get the timestep at which the sediment input event occurs. This
         // may be specified either as a relative time (i.e. a number of hours or
         // days after the simulation start) or as an absolute time (i.e. a
         // time/date in the format hh-mm-ss dd/mm/yyyy)
         unsigned long const ulEventTimeStep = ulConvertToTimestep(&VstrTmp[1]);

         if (ulEventTimeStep == SEDIMENT_INPUT_EVENT_ERROR)
         {
            strErr = "line " + to_string(nLine) + ": invalid time and/or date '" +
               VstrTmp[1] + "' for sediment input event in " +
               m_strSedimentInputEventFile;
            break;
         }

         // Then the volume (m3) of fine sediment, first check that this is a valid
         // double
         ...
         if (dFineSedVol < 0)
         {
            strErr = "line " + to_string(nLine) + ": negative number '" +
               to_string(dFineSedVol) +
               "' for fine sediment volume for sediment input event in " +
               m_strSedimentInputEventFile;
            break;
         }
         ...
         // Then the volume (m3) of sand sediment, first check that this is a valid
         // double
         if (! bIsStringValidDouble(VstrTmp[3]))
         {
            strErr =
               "line " + to_string(nLine) + ": invalid floating point number '" +
               VstrTmp[3] +
         ...
         {
            strErr =
               "line " + to_string(nLine) + ": negative number '" +
               to_string(dSandSedVol) +
               "' for sand-sized sediment volume for sediment input event in " +
               m_strSedimentInputEventFile;
            break;
         }
         ...
         // Then the volume (m3) of coarse sediment, first check that this is a
         // valid double
         ...
         // Only read the last two items if we have on-coast sediment block
         // sediment input
         double dLen = 0;
         double dWidth = 0;
         ...
         {
            // The coast-normal length (m) of the sediment block, first check that
            ...
            {
               strErr = "line " + to_string(nLine) +
                  ": coast-normal length of the sediment block '" +
                  to_string(dLen) + "' must be > 0 in " +
                  m_strSedimentInputEventFile;
               break;
            }

            // The along-coast width (m) of the sediment block, first check that
            // this is a valid double
            ...
            {
               strErr = "line " + to_string(nLine) +
                  ": along-coast width (m) of the sediment block '" +
                  to_string(dWidth) + "' must be > 0 in " +
                  m_strSedimentInputEventFile;
               break;
            }

            // // The along-coast thickness (m) of the sediment block, first check
            // that this is a valid double if (! bIsStringValidDouble(VstrTmp[7]))
            // {
            // strErr = "line " + to_string(nLine) + ": invalid floating point
            // number '" + VstrTmp[7] + "' for along-coast thickness of sediment
            // input event in " + m_strSedimentInputEventFile; break;
            // }
            //
            // dThick = stod(strTrim(&VstrTmp[7]));
            // if ((! m_bSedimentInputAtCoast) && (dWidth <= 0))
            // {
            // strErr = "line " + to_string(nLine) + ": along-coast thickness (m) of
            // the sediment block '" + to_string(dThick) + "' must be > 0 in " +
            // m_strSedimentInputEventFile; break;
            // }
         }

         // Create the CSedInputEvent object
         CSedInputEvent *pEvent =
            new CSedInputEvent(nID, ulEventTimeStep, dFineSedVol, dSandSedVol,
                               dCoarseSedVol, dLen, dWidth);      //, dThick);
   ...
   {
      // Error in input to initialisation file
      cerr << ERR << strErr << " in sediment input event file "
           << m_strSedimentInputEventFile << endl
           << "'" << strRec << "'" << endl;
      InStream.close();
