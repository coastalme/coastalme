...
bool CSimulation::bWriteVectorGISFile(int const nDataItem, string const* strPlotTitle)
{
   ...
   case (VECTOR_PLOT_FLOOD_LINE):
   {
      ...
      if (m_bFloodSWLSetupSurgeRunupLine)
      {
         ...
         {
            cerr << ERR << "cannot create " << strType << " attribute field 5 '" << strFieldValue4 << "' in " << strFilePathName << endl
                 << CPLGetLastErrorMsg() << endl;
            return false;
         }
   ...
   case (VECTOR_PLOT_INVALID_NORMALS):
   {
      ...
      if (pOGRLayer->CreateField(&OGRField7) != OGRERR_NONE)
      {
         cerr << ERR << "cannot create " << strType << " attribute field 7 '" << strFieldValue7 << "' in " << strFilePathName << endl
              << CPLGetLastErrorMsg() << endl;
         return false;
      }
      ...
      if (pOGRLayer->CreateField(&OGRField8) != OGRERR_NONE)
      {
         cerr << ERR << "cannot create " << strType << " attribute field 8 '" << strFieldValue8 << "' in " << strFilePathName << endl
              << CPLGetLastErrorMsg() << endl;
         return false;
      }
