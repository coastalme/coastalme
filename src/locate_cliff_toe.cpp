...
void CSimulation::nCalcSlopeAtAllCells(void)
{
   for (int nX = 0; nX < m_nXGridSize; nX++)
   {
      for (int nY = 0; nY < m_nYGridSize; nY++)
      {
         ...
         {
            double const dElevLeft = m_pRasterGrid->Cell(nX - 1, nY).dGetSedimentTopElev();
            double const dElevRight = m_pRasterGrid->Cell(nX + 1, nY).dGetSedimentTopElev();
            double const dElevUp = m_pRasterGrid->Cell(nX, nY - 1).dGetSedimentTopElev();
            double const dElevDown = m_pRasterGrid->Cell(nX, nY + 1).dGetSedimentTopElev();
   ...
   for (int i = 0; i < nSize - 1; i++)
   {
      ...
      if (bIsWithinValidGrid(nLeftX, nLeftY) && bIsWithinValidGrid(nRightX, nRightY))
      {
         ...
         if (bLeftIsCliff != bRightIsCliff)
         {
            ...
            double const dLeftElev = m_pRasterGrid->Cell(nLeftX, nLeftY).dGetSedimentTopElev();
            double const dRightElev = m_pRasterGrid->Cell(nRightX, nRightY).dGetSedimentTopElev();
