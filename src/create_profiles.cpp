         ...
         if (! pProfile->bIsGridEdge())
         {
            ...
            if (m > PROFILE_CHECK_DIST_FROM_COAST)
            {
               ...
               if (! m_pRasterGrid->Cell(nX, nY).bIsInContiguousSea())
               {
                  ...
                  LogStream << m_ulIter << ": profile " << nProfile << " HIT LAND at [" << nX << "][" << nY << "] = {" << dGridCentroidXToExtCRSX(nX) << ", " << dGridCentroidYToExtCRSY(nY) << "}, elevation = " << m_pRasterGrid->Cell(nX, nY).dGetSedimentTopElev() << ", SWL = " << m_dThisIterSWL << endl;
               ...
               if (m_pRasterGrid->Cell(nX, nY).nGetInterventionClass() != INT_NODATA)
               {
                  ...
                  LogStream << m_ulIter << ": profile " << nProfile << " HIT INTERVENTION at [" << nX << "][" << nY << "] = {" << dGridCentroidXToExtCRSX(nX) << ", " << dGridCentroidYToExtCRSY(nY) << "}, elevation = " << m_pRasterGrid->Cell(nX, nY).dGetSedimentTopElev() << ", SWL = " << m_dThisIterSWL << endl;
