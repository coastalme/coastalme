...
void CSimulation::LocateAndCreateProfiles(int const nCoast, int& nProfile, vector<bool>* pbVCoastPointDone, vector<pair<int, double>> const* prVCurvature)
{
        ...
        n--)
   {
      ...
      if (! pbVCoastPointDone->at(nNormalPoint))
      {
         ...
         // // DEBUG CODE =================================================================================
         // if (m_pRasterGrid->Cell(PtiThis.nGetX(), PtiThis.nGetY()).bIsCoastline())
         // LogStream << m_ulIter << ": cell[" << PtiThis.nGetX() << "][" << PtiThis.nGetY() << "] IS coastline, coast number = " << m_pRasterGrid->Cell(PtiThis.nGetX(), PtiThis.nGetY()).nGetCoastline() << endl;
         // else
...
int CSimulation::nCreateProfile(int const nCoast, int const nCoastSize, int const nProfileStartPoint, int const nProfile, bool const bIntervention, CGeom2DIPoint const* pPtiStart)
{
   ...
   if (m_pRasterGrid->Cell(nXEnd, nYEnd).dGetSeaDepth() < m_dDepthOfClosure)
   {
      // if (m_nLogFileDetail >= LOG_FILE_ALL)
...
int CSimulation::nLocateAndCreateGridEdgeProfile(bool const bCoastStart, int const nCoast, int& nProfile)
{
   ...
   for (unsigned int n = 0; n < VPtiNormalPoints.size(); n++)
   {
      ...
      m_pRasterGrid->Cell(nX, nY).SetCoastAndProfileID(nCoast, nProfile);
   ...
   double const dDeepWaterWaveHeight = m_pRasterGrid->Cell(nEndX, nEndY).dGetCellDeepWaterWaveHeight();
   double const dDeepWaterWaveAngle = m_pRasterGrid->Cell(nEndX, nEndY).dGetCellDeepWaterWaveAngle();
   double const dDeepWaterWavePeriod = m_pRasterGrid->Cell(nEndX, nEndY).dGetCellDeepWaterWavePeriod();
...
int CSimulation::nCheckAndMarkAllProfiles(void)
{
   ...
   for (unsigned int nCoast = 0; nCoast < m_VCoast.size(); nCoast++)
   {
      ...
      // {
      //
      // //         pdRaster[nn] = m_pRasterGrid->Cell(nX, nY).nGetPolygonID();
      // pdRaster[nn] = m_pRasterGrid->Cell(nX, nY).nGetProfileID();
      // }
      //
      // nn++;
      // }
...
void CSimulation::MarkProfilesOnGrid(int const nCoast, int& nValidProfiles)
{
   ...
   for (int n = 0; n < nProfiles; n++)
   {
      ...
      for (unsigned int k = 0; k < VCellsToMark.size(); k++)
      {
         ...
         m_pRasterGrid->Cell(nXTmp, nYTmp).SetCoastAndProfileID(nCoast, nProfile);
      ...
      // Get the deep water wave height and orientation values at the end of the profile
      double const dDeepWaterWaveHeight = m_pRasterGrid->Cell(VCellsToMark.back().nGetX(), VCellsToMark.back().nGetY()).dGetCellDeepWaterWaveHeight();
      double const dDeepWaterWaveAngle = m_pRasterGrid->Cell(VCellsToMark.back().nGetX(), VCellsToMark.back().nGetY()).dGetCellDeepWaterWaveAngle();
      double const dDeepWaterWavePeriod = m_pRasterGrid->Cell(VCellsToMark.back().nGetX(), VCellsToMark.back().nGetY()).dGetCellDeepWaterWavePeriod();
   ...
   for (nSeg = 0; nSeg < nNumSegments; nSeg++)
   {
      ...
      for (int m = 0; m <= nRound(dLength); m++)
      {
         ...
         if (! pProfile->bIsGridEdge())
         {
            ...
            if (m_pRasterGrid->Cell(nX, nY).bIsProfile() || m_pRasterGrid->Cell(nX, nYTmp).bIsProfile())
            {
               // This cell or an adjacent cell, is 'under' a profile, so now check if the profile belongs to another coast
               int const nHitProfileCoast1 = m_pRasterGrid->Cell(nX, nY).nGetProfileCoastID();
               int const nHitProfileCoast2 = m_pRasterGrid->Cell(nX, nYTmp).nGetProfileCoastID();
            ...
            if (m > PROFILE_CHECK_DIST_FROM_COAST)
            {
               ...
               if (! m_pRasterGrid->Cell(nX, nY).bIsInContiguousSea())
               {
                  ...
                  LogStream << m_ulIter << ": \tcoast " << nCoast << " profile " << nProfile << " HIT LAND at [" << nX << "][" << nY << "] = {" << dGridCentroidXToExtCRSX(nX) << ", " << dGridCentroidYToExtCRSY(nY) << "}, elevation = " << m_pRasterGrid->Cell(nX, nY).dGetAllSedTopElevIncTalus() << ", SWL = " << m_dThisIterSWL << endl;

                  return;
               }

               if (m_pRasterGrid->Cell(nX, nY).nGetInterventionClass() != INT_NODATA)
               {
                  // We've hit an intervention, so set a switch and mark the profile
                  bHitIntervention = true;
                  pProfile->SetHitIntervention(true);

                  LogStream << m_ulIter << ": \tcoast " << nCoast << " profile " << nProfile << " HIT INTERVENTION at [" << nX << "][" << nY << "] = {" << dGridCentroidXToExtCRSX(nX) << ", " << dGridCentroidYToExtCRSY(nY) << "}, elevation = " << m_pRasterGrid->Cell(nX, nY).dGetAllSedTopElevIncTalus() << ", SWL = " << m_dThisIterSWL << endl;
            ...
            if (m_pRasterGrid->Cell(nX, nY).bIsProfile())
            {
               ...
               int const nHitProfileCoast = m_pRasterGrid->Cell(nX, nY).nGetProfileCoastID();
