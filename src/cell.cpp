...
CGeomCell::CGeomCell()
    : m_bInContiguousSea(false), m_bInContiguousFlood(false),
      ...
      m_dInterventionHeight(0), m_dInterventionTriggerElev(0) {
...
//! the profile. Otherwise it returns INT_NODATA
...
//! Returns the global ID number of the polygon which 'contains' this cell
//! (returns INT_NODATA if the cell is not 'in' a polygon)
...
//! INT_NODATA if the cell is not 'in' a polygon)
...
bool CGeomCell::bIsElevLessThanWaterLevel(void) const {
  double dSedTopElev = m_VdAllHorizonTopElev.empty()
                           ? m_dBasementElevation
                           : m_VdAllHorizonTopElev.back();
  return ((dSedTopElev + m_dInterventionHeight) <
...
double CGeomCell::dGetSedimentPlusInterventionTopElev(void) const {
  if (m_VdAllHorizonTopElev.empty())
    return m_dBasementElevation + m_dInterventionHeight;
...
double CGeomCell::dGetOverallTopElev(void) const {
  if (m_VdAllHorizonTopElev.empty())
    return m_dBasementElevation + m_dInterventionHeight + m_dSeaDepth;
...
void CGeomCell::CalcAllLayerElevsAndD50(void) {
  ...
  for (int n = static_cast<int>(m_VLayerAboveBasement.size()) - 1; n >= 0;
    ...
    if (dUnconsThick > 0) {
      ...
      double const dCoarseProp =
          pUnconsSedLayer->dGetCoarseDepth() / dUnconsThick;
...
//! elevation (layer 0 being the first above the basement). Note that the
...
void CGeomCell::SetSeaDepth(void) {
  ...
  m_dSeaDepth = tMax(m_pGrid->pGetSim()->CSimulation::dGetThisIterSWL() -
                         (dSedTopElev + m_dInterventionHeight),
                     0.0);
}
...
//! Sets the intervention trigger elevation based on depth below current
//! sediment surface
...
//! Returns true if intervention should fail (sediment top is below trigger
//! elevation)
bool CGeomCell::bInterventionShouldFail(void) const {
  // Only check if we have an intervention and a valid trigger elevation
  if (m_Landform.nGetLFCategory() != LF_CAT_INTERVENTION ||
      bFPIsEqual(m_dInterventionTriggerElev, DBL_NODATA, TOLERANCE))
    return false;
